...
async function createPaymentLink(token: string, body: {
  ...
  if (!data.payment_link) throw new Error("Error creando payment link: " + JSON.stringify(data));
...
serve(async (req) => {
  ...
  // ================================================
  // POST /create-session — crea Payment Link (URL de pago real)
  // ================================================
  if (req.method === "POST" && path === "/create-session") {
    try {
      ...
      const link  = await createPaymentLink(token, {
        ...
        description:      "Abono Mini-Vac - " + (nombre || folio || lead_id || ""),
        customer_email:   email || undefined,
        customer_name:    nombre || undefined,
      });
